...
def compute(df: pd.DataFrame) -> pd.DataFrame:
    ...
    future_ratio_5d_t1 = df["ref_price"].shift(-6) / df["ref_price"].shift(-1)
    valid_target_5d_t1 = (
        future_ratio_5d_t1.notna()
        & np.isfinite(future_ratio_5d_t1)
        & (future_ratio_5d_t1 > 0.05)
        & (future_ratio_5d_t1 < 20.0)
    )
